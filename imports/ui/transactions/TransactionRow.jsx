...
export const TransactionRow = (props) => {
    ...
    return <SentryBoundary><Row className={(tx.code)?"tx-info invalid px-6 py-3":"tx-info px-6 py-3"}>
        <Col xs={12} lg={homepage ? 5 : 7} className="activity" >{(tx?.tx?.body?.messages && tx?.tx?.body?.messages.length >0)?tx?.tx?.body?.messages.map((msg,i) => {
            return <Card className="px-4 py-2 border-0 bg-gray-light" body key={i}><Activities msg={msg} id={props.index} invalid={(!!tx.tx_response.code)} events={(tx.tx_response.logs&&tx.tx_response.logs[i])?tx.tx_response.logs[i].events:null} /></Card>
        }):''}</Col>
